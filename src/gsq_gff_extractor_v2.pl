...
my ($est_id,$chr_id,$exon_start,$exon_end,$exon_orientation,$species_chr_id,$gene_orientation,$gene_start,$gene_end,$gene_id,$gene_chr);
my (@exon_positions,@gff_coords,%est_start_to_est_ID,%est_start_to_chr,%start_to_id,%id_to_allstarts,%gsq_start_orientation,%gff_start_orientation,%exon_start_end,%gff_start_chr) = ();

while(<INFILE_TRANS>)
{
#Sequence    1:   4, from 1 to 300000, both strands analyzed.
 if(/^Sequence.*:\t*\s*([a-zA-Z_0-9]+),\s*from.*$/)
 {
  #print "$1\n";
  $chr_id = $1;
 }
# EST sequence     12 -strand   942 n (File: 42517803-)
 if(/^EST sequence.*strand.*\(File:\s*(\d+)[+-]*\)$/)
 {
  #print "$1\n";
  $est_id = $1; 
 }
...
for(my $i=0;$i<=$#exon_positions;$i++)      ####### Associate 5'-transcript ends with gene IDs based on distance from 5'-annotated gene start and orientation
{
 for (my $j=0;$j<=$#gff_coords;$j+=2)
 {
  if($gff_start_orientation{$gff_coords[$j]} eq '+')
  {
   ...
   {
    print OUTFILE "chr$gff_start_chr{$gff_coords[$j]}\t$exon_positions[$i]\t$exon_positions[$i]\tgi|$est_start_to_est_ID{$exon_positions[$i]}\t.\t$gff_start_orientation{$gff_coords[$j]}\n";
    last;
   }
  }
