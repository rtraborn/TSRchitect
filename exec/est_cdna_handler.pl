...
#### USAGE --- perl est_cdna_handler.pl -trans <cDNA_EST_dataset_to_be used_for_spliced_alignment> -seq <Nucleotide_sequence_to_be_used_for_spliced_alignment> -gff <GFF_annotation_file> -chrnum <Chromosome_number_of_sequence> -gsq [GeneSeqer_output_file] -dist [Distance_from_annotated_gene_start_to_be_used_for_capturing_relevant_transcripts] -out [Cluster-format-file] -locn [Location_of_bin_folder_of_GeneSeqer] -species [Species_name]
...
my ($infile_trans,$infile_seq,$infile_gff,$chrnum,$infile_gsq,$infile_gth,$outfile,$gsq_locn,$gth_locn,$species,$dist);
...
if(defined $infile_gsq) {}		##### If there is a readymade gsq file, no need for all other commands
elsif(defined $infile_gth) {}
else { if(defined $infile_trans && defined $infile_seq && defined $infile_gff && defined $chrnum) {} else { print $usage; exit; } }   ##### Show usage if cmd-line parameters are undefined
...
my @outname_temp = split (/\//,$infile_seq);
my $outlabel = $outname_temp[$#outname_temp];
my $gsq_name = "gsq." . $outlabel;
my $gth_name = "gth." . $outlabel;
if(defined $outfile) {} else { $outfile = $infile_seq. "_Cluster_Format"}

if(defined $infile_gsq) { `perl gsq_gff_extractor.pl -gsq $infile_gsq -gff $infile_gff -chrnum $chrnum -dist $dist -out $outfile`; }
elsif(defined $infile_gth) { `perl gsq_gff_extractor.pl -gth $infile_gth -gff $infile_gff -chrnum $chrnum -dist $dist -out $outfile`; }
else 
{
 if(defined $gsq_locn)
 {
  ...
  `perl gsq_gff_extractor.pl -gsq $gsq_name -gff $infile_gff -chrnum $chrnum -dist $dist -out $outfile`;
#  `rm $gsq_name`;
 }
 elsif(defined $gth_locn)
 {
  `"$gth_locn/"gth -genomic $infile_seq -cdna $infile_trans -o $gth_name`;
  `perl gsq_gff_extractor.pl -gth $gth_name -gff $infile_gff -chrnum $chrnum -dist $dist -out $outfile`; 
#  `rm $gth_name`;
 }
}
