...
# Removed the filter section just for now 7/23/13
# Author: Taylor Raborn with Krishna Sridharan

###############################################################################
TSR_Completev2i <- function(TSS_data,binWidth=10,count=1,cluster_Num=1,Draw=FALSE,print=FALSE, countMin=25) {
...
#			which(is.na(remove_index)) -> keep_index
		TSS_data -> TSS_data_i #keeping the rows from TSS_data that are more than the specified countMin value
...
TSR_packagev2 <- function(TSS_data,gene=1,clusterNum=1,binWidth=10,count=1,draw=TRUE) {
	...
	options(scipen=999) #I'm just not a fan of scientific notation here
	 ...
	 var_calc_i <- function(x, gene, iterations=1000000) {
 		gene -> gene_name
 		which(as.character(x[,2])==gene) -> this_gene_index
...
#}
	...
	as.character(TSS_data_i[,2]) -> genes_vector
